...
def main():
    ...
    if args.wandb:
        try:
            ...
            wandb_run = wandb.init(
                ...
                name=f"{config['model_name']}_{timenow}",
                config=config,
                tags=[config['model_name'], timenow]
            )
    ...
    if args.debug:
        ...
        logger.info("Debug mode enabled: 3 epochs, 1/100 train data, 1/10 test data")
    else:
        config['debug'] = False
    ...
    # Save training dynamics (Dataset Cartography)
    ...
    print(f"TRAINING_LOG_DIR={logger_path}")
