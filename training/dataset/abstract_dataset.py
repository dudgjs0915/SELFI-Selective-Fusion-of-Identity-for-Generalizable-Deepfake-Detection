...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def __init__(self, config=None, mode='train'):
        ...
        # Two-phase training: Filter by allowed_frames if provided
        if config.get('allowed_frames') is not None:
            allowed_frames = config['allowed_frames']
            original_size = len(self.image_list)
            
            # Filter image_list and label_list
            filtered_image_list = []
            filtered_label_list = []
            ...
            for idx, img_path in enumerate(self.image_list):
                ...
                if isinstance(img_path, list):
                    # Video-level mode: check if any frame in the clip is in allowed_frames
                    if any(frame in allowed_frames for frame in img_path):
                        filtered_image_list.append(img_path)
                        filtered_label_list.append(self.label_list[idx])
                else:
                    # Image-level mode: check if frame is in allowed_frames
                    if img_path in allowed_frames:
                        filtered_image_list.append(img_path)
                        filtered_label_list.append(self.label_list[idx])
            
            self.image_list = filtered_image_list
            self.label_list = filtered_label_list
            
            filtered_size = len(self.image_list)
            print(f"Phase 1 filtering: {original_size} -> {filtered_size} samples ({filtered_size/original_size*100:.1f}%)")
            
            # Count by label
            from collections import Counter
            label_counts = Counter(self.label_list)
            print(f"  Label distribution after filtering: {dict(label_counts)}")

        # Debug mode: sample a subset of data
        if config.get('debug', False):
            if mode == 'train':
                sample_ratio = config.get('train_sample_ratio', 0.01)
            else:  # test
                sample_ratio = config.get('test_sample_ratio', 0.1)
            ...
            print(f"Debug mode: {mode} data sampled from {total_samples} to {num_samples} ({sample_ratio*100:.1f}%)")
    ...
    def __getitem__(self, index, no_norm=False):
        ...
        # Return image paths for training dynamics tracking
        # For video level, use the first frame path; for image level, use the single path
        sample_name = image_paths[0] if self.video_level else image_paths[0]

        return image_tensors, label, landmark_tensors, mask_tensors, sample_name
    ...
    def collate_fn(batch):
        ...
        Args:
            batch (list): A list of tuples containing the image tensor, the label tensor,
                          the landmark tensor, the mask tensor, and the sample name.

        Returns:
            A tuple containing the image tensor, the label tensor, the landmark tensor,
            and the mask tensor.
        """
        # Separate the image, label, landmark, mask tensors, and names
        ...
        data_dict['name'] = names  # Add sample names for tracking
        return data_dict
